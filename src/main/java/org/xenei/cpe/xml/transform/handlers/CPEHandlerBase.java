...
//	public static final String meta="http://scap.nist.gov/schema/cpe-dictionary-metadata/0.2";
	...
	/**
	 * Push a handler onto the stack. The new handler will begin receiving SAX
	 * events.
	 * 
	...
	private void addCPEValue(Resource subject, Property property, String value) {
		if (value.equals(LogicalValue.ANY.getAbbreviation()) || value.equals(LogicalValue.NA.getAbbreviation())) {
			return;
		}
	...
	protected <T extends Enum<T>> void addOptionalAttribute(Resource subject, Class<T> enumType, Attributes attributes,
			String name, Property property) throws SAXException {
		String value = attributes.getValue(name);
		if (value != null) {
			try {
	...
	protected void addRequiredAttribute(Resource subject, Attributes attributes, String name, Property property)
			throws SAXException {
		String value = attributes.getValue(name);
		if (value == null) {
	...
	protected void addRequiredURLAttribute(Resource subject, Attributes attributes, String name, Property property)
			throws SAXException {
		String value = attributes.getValue(name);
		if (value == null) {
			throw new SAXException(name + " is a requried attribute");
	...
	protected <T extends Enum<T>> void addRequiredAttribute(Resource subject, Class<T> enumType, Attributes attributes,
			String name, Property property) throws SAXException {
		String value = attributes.getValue(name);
		if (value == null) {
			throw new SAXException(name + " is a requried attribute");
	...
	/**
	 * Parse the cpe name into a CPE and create a URL from it. Add all the defined
